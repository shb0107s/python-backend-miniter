...
@pytest.fixture
@mock.patch("app.boto3")  # app.py에서 import한 boto3 모듈을 patch하겠다는 뜻
def api(mock_boto3):  # 위에서 patch한 mock객체를 인자로 받음. 인자의 이름은 자유.
...
def test_login(api):
    resp = api.post(
        '/login',
        data=json.dumps({'email': 'shb0107@gmail.com', 'password': 'test password'}),
...
def test_upload_and_get_profile_picture(api):
    # 로그인
    resp = api.post(
        '/login',
        data=json.dumps({'email': 'shb0107@gmail.com', 'password': 'test password'}),
